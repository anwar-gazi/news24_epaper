                        ...
                        <center>
                            <div class="pagination" style="margin: 0px;padding: 5px">
                                <a style="margin-left: 0px;"
                                    href="{{ url('/' . $page_edition . '/' . $date . '/' . $page_last) }}">&laquo;</a>
                                @for ($i = 1; $i <= count($pagination_pages); $i++)
                                    <a class="{{ $i == $current_page ? 'active' : '' }}"
                                        href="{{ url('/' . $page_edition . '/' . $date . '/' . $i) }}">{{ $i }}</a>
                                @endfor
        ...
        <div class="left-content">
                        ...
                        @if (!empty($epaper_articles) && count($epaper_articles) > 0)
                            @foreach ($epaper_articles as $key => $article)
                                @php
                                    ...
                                    $get_image_width = \App\Epaper::GetImageSize(
                                        $image_width_location . $article->image,
                                    );
                                @endphp

                                <area shape="rect" coords="{{ $article->coords }}" data-image="{{ $article->image }}"
                                    class="main-img"
                                    onclick="modalOpen('<?php echo $article->image; ?>','<?php echo $image_location; ?>','<?php echo $related_item; ?>','<?php echo $article->relation; ?>', '<?php echo $get_image_width; ?>')" />
                            @endforeach
                        @endif
                    </map>
            ...
            <table width="100%" class="page-trigger" style="padding: 10px 10px 0px 10px;margin-left: 0px">
                <tr>
                    <td>
                        @if ($page_last + 1 > 1)
                            <a style="float: left" href="{{ url('/' . $page_edition . '/' . $date . '/' . $page_last) }}"><img
                                    src="{{ asset('assets/images/front/previous.png') }}" /></a>
                        @else
                            @if (!empty($date))
                                <a style="float: left" href="{{ url('/all/pages/' . $page_edition . '/' . $date) }}"
                    ...
                    <td>
                        @if (count($get_categories) > $page_next - 1)
                            <a href="{{ url('/' . $page_edition . '/' . $date . '/' . $page_next) }}" class="pull-right"><img
                ...
                <table width="100%" class="modal_table">
                    <tr>
                        ...
                        <td class="text-center">
                            <p>
                                <a href="{{ url('/') }}"><img src="{{ asset('assets/images/logo1.png') }}"
        ...
        function printDiv(bangla_date) {
            ...
            if (related_image_link != '') {
                newWin.document.write('<html><body onload="window.print()">' +
                    '<center><img src="{{ asset('assets/images/logo1.pngg') }}" style="height:100px;" />' +
