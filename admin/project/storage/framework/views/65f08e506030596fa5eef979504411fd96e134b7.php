...
<form class="form-horizontal" action="<?php echo e(url('/page/update',$page_info->page_id)); ?>" method="post" enctype="multipart/form-data">
	<input type="hidden" name="_token" value="<?php echo e(csrf_token()); ?>">
	<input type="hidden" name="publish_date" value="<?php echo e($page_info->publish_date); ?>">
		...
		<div class="col-sm-7">
			...
			<div>
				<select id="editonSelectionEdit" multiple="multiple" class="edition_get_edit">
					...
					<?php  $editions_get=\App\Page::PageEdition($page_info->publish_date, $page_info->page_id);  ?>
					<?php if(!empty($editions_get) && (count($editions_get)>0)): ?>
					<?php  $check_edition=0; ?>
					<?php foreach($editions_get as $key => $page_edition): ?>
					<?php if($page_edition->id==$editions->id): ?>
					<?php  $check_edition=1; ?>
					<?php endif; ?>
					<?php endforeach; ?>
					<?php endif; ?>
					<option value="<?php echo e($editions->id); ?>" <?php echo e(isset($check_edition) && ($check_edition==1) ? 'selected' : ''); ?>><?php echo e($editions->name); ?></option>
					<?php endforeach; ?>
					<?php endif; ?>
				</select>
	...
	<div class="form-group">
		...
		<div class="col-sm-7">
			<select class="form-control" name="category" >
				...
				<option value="<?php echo e($categories->id); ?>" <?php echo e(isset($page_info) && ($page_info->category_id==$categories->id) ? 'selected' : ''); ?>><?php echo e($categories->name); ?></option>
				<?php endforeach; ?>
				<?php endif; ?>
			</select>
		</div>
	...
	<div class="form-group">
		...
		<div class="col-sm-7">
			<select class="form-control" name="page_number" required="">
				...
				<option value="<?php echo e($i); ?>" <?php echo e(isset($page_info) && ($page_info->page_number==$i) ? 'selected' : ''); ?> >Page <?php echo e(str_pad($i, 2, "0", STR_PAD_LEFT)); ?></option>
				<?php endfor; ?>
			</select>
		</div>
	</div>
